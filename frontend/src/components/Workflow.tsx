...
import { AgentNode, AskAIAgent, ChatModelAgent, ChatTriggerAgent, FlowType, getFlowType, MemoryAgent } from '../nodeTypes';
...
const getPositionForType = (type: string) => {
  switch (type) {
    case 'chatTrigger':
...
const Workflow: React.FC = () => {
    ...
    const getAgents = useCallback(async (data: { map: (arg0: (agentData: AgentResponse) => { id: string; type: string; data: { next_agents: string[]; memory_node: undefined; chat_model_node: undefined; retriever_node: undefined; status: string; flowType: FlowType; id: string; slug: string; type: string; workflow: string; properties: BaseProperties; connections_out: ConnectionResponse[]; memories: ChatMemory[]; }; position: { x: number; y: number; }; }) => AgentNode<keyof { documentLoader: typeof DocumentLoaderAgent; chat: AskAIAgent; chatTrigger: ChatTriggerAgent; memory: MemoryAgent; chatModel: ChatModelAgent; vectorStore: typeof VectorStoreNode; }>[]; }): Promise < void > => {
        try {
    ...
    const handleUpdateChatInput = (e: ChangeEvent<HTMLTextAreaElement>) => {
    ...
    const handleResetWorkflow = async () => {
        const { data } = await api.post(`/api/reset-workflow/${id}/`);
        getAgents(data);
    }
  ...
  <div className="h-screen overflow-hidden flex flex-col bg-gray-900 text-gray-100">
    ...
    <div className="flex-1 min-h-0 grid grid-rows-2 grid-cols-[70%_30%] overflow-hidden">
      ...
      <div className="border border-gray-700 bg-gray-800 text-gray-100 flex flex-col h-full">
        ...
        <div ref={logsContainerRef} className="p-4 overflow-y-auto flex-1 min-h-0">
          {logs.map((log, index) => (
            <div key={index} className={`text-sm mb-2 ${log.type === 'error' ? 'text-red-400' : log.type === 'warning' ? 'text-yellow-400' : 'text-gray-300'}`}>
      ...
      <div className="border border-gray-700 flex flex-col min-h-0">
        <div ref={chatContainerRef} className="flex-1 overflow-y-auto min-h-0 p-4">
          ...
          {activeNode && <div className="text-sm text-gray-400 mb-4 text-center">{activeNode.id}</div>}
      ...
      <div className="flex flex-col justify-between w-full min-h-0 h-full">
        <div className="p-4 w-full">
          ...
          {activeDocumentLoader && <div className="text-sm text-gray-400 mb-4 text-center">{activeDocumentLoader.id}</div>}
          <div className="w-full h-48 mt-8 border-1 rounded-lg">
            <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">PDF, DOCX, MD, TXT, JSON</p>
              </div>
